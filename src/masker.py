...
class TokenMasker:
    def __init__(self, llm: LLM) -> None:
        ...
        S = JSONState
        self.state_handlers = {
            S.EXPECT_OBJECT_START: lambda p,
            c: self._get_tokens_for_string("{", p),
            S.EXPECT_ARRAY_START: lambda p,
            c: self._get_tokens_for_string("[", p),
            S.EXPECT_COLON: lambda p,
            c: self._get_tokens_for_string(":", p),

            S.EXPECT_KEY: lambda p,
    ...
    def _get_number_tokens(self, is_empty_prefix: bool, has_digits: bool,
        ...
        def is_valid_num(s: str) -> bool:
            ...
            if not has_digits and any(c in allowed_chars for c in s):
                return False
            return all(c in f"0123456789.-{allowed_chars}"
