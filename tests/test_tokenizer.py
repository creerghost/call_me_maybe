...
TEST_STRINGS = [
    ...
    "And some completely random string to ensure the numpy vectorization works perfectly!",
    "     Leading spaces and trailing spaces    ",
    "Testing\nnewlines\nand tabs",
]

@pytest.mark.parametrize("text", TEST_STRINGS)
def test_encode_matches_huggingface(tokenizers, text):
...
@pytest.mark.parametrize("text", TEST_STRINGS)
def test_decode_matches_huggingface(tokenizers, text):
    custom_tokenizer, hf_tokenizer = tokenizers
    ...
    assert custom_decoded == hf_decoded, f"Decoded strings do not match for '{text}'"
    assert custom_decoded == text, f"Decoded string does not match original text"
